...
func start():
	...
	BattlefieldInfo.message_system.no_more_text.connect(move_actor)
	
	# Register Movement
	BattlefieldInfo.movement_system_cinematic.unit_finished_moving_cinema.connect(move_cursor)
